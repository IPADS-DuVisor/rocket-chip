...
class TLBEntry(val nSectors: Int, val superpage: Boolean, val superpageOnly: Boolean)(implicit p: Parameters) extends CoreBundle()(p) {
  ...
  def hit(vpn: UInt, vs1: Bool, vs2: Bool) = {
    if (superpage && usingVM) {
      var tagMatch = valid.head & (entry_data.head.vstage1 === vs1) && (entry_data.head.vstage2 === vs2)
      ...
      val virtualMatch = entry_data.map(x => x.vstage1 === vs1 && x.vstage2 === vs2)
...
class TLB(instruction: Boolean, lgMaxSize: Int, cfg: TLBConfig)(implicit edge: TLEdgeOut, p: Parameters) extends CoreModule()(p) {
  ...
  val superpage_hits = superpage_entries.map(_.hit(vpn, stage1_en && priv_v, stage2_en))
  ...
  def badVA(guestPA: Boolean): Bool = {
    ...
    val extraBits = if (guestPA) (hypervisorExtraAddrBits + 1) else 0
