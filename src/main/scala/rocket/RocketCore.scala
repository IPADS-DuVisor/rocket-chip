...
class Rocket(tile: RocketTile)(implicit p: Parameters) extends CoreModule()(p)
  ...
  val decode_table_special = {
    Seq(new SpecialDecode)
  } flatMap(_.table)
  ...
  val id_ctrl_all = Wire(new IntCtrlSigs()).decode(id_inst(0), decode_table)
  val id_ctrl_special = Wire(new IntCtrlSigs()).decode(id_inst(0), decode_table_special)
  val id_ctrl = Mux(id_ctrl_special.legal, id_ctrl_special, id_ctrl_all)
  ...
  // val (_, ex_reg_gpaddr) := checkExceptions(List(
  //   (id_xcpt0.gf.inst, id_xcpt0.gpaddr),
  //   (id_xcpt1.gf.inst, id_xcpt1.gpaddr)
  // ))
  
  ex_reg_gpaddr := Mux(id_xcpt0.gf.inst, id_xcpt0.gpaddr, id_xcpt1.gpaddr)
  ...
  val htval_valid = Bool(usingHypervisor) && wb_xcpt && wb_cause.isOneOf(Causes.store_guest_page_fault,
  ...
  val wb_gpaddr = Mux(wb_reg_xcpt, wb_reg_gpaddr , io.dmem.s2_xcpt.gpaddr)
  ...
  io.dmem.req.bits.addr := Mux(ex_reg_mmio_flag, alu.io.adder_out, encodeVirtualAddress(ex_rs(0), alu.io.adder_out))
  io.dmem.req.bits.idx.foreach(_ := io.dmem.req.bits.addr)
  io.dmem.req.bits.dprv := Mux(ex_reg_mmio_flag, PRV.M, Mux(ex_reg_hls, csr.io.hstatus.spvp, csr.io.status.dprv))
  io.dmem.req.bits.dv := Mux(ex_reg_mmio_flag, false.B, Mux(ex_reg_hls, true.B, csr.io.status.dv))
  ...
  def encodeVirtualAddress(a0: UInt, ea: UInt) = if (vaddrBitsExtended == vaddrBits) ea else {
    ...
    val guest_pa = Bool(usingHypervisor) && csr.io.status.v && 
        csr.io.vsatp.mode === 0.U && csr.io.hgatp.mode =/= 0.U
    Mux(guest_pa, {
